 ...
 * @author diegorivera
 */
public class SetAttributeType implements Command{
	...
	public CommandReturn execute(Set<Policy> policies) throws CommandExecutionException{
		...
		for (PolicyAttribute pa : policy.getAttributes()){
			if (pa.getPath().equals(this.attributePath)){
				...
				message = "Attribute in policy ID " + policy.getPolicyID() + " for path " + pa.getPath() + " was updated succesfully";
				updated = true;
			}
		}
		...
		if (!updated){
			...
			message = "An attribute for path " + this.attributePath + " in policy " + policy.getPolicyID() + " was added";
		}
	...
	public boolean parseCommandArgs(String[] args) throws CommandParserException{
		...
		try{
			...
			throw new CommandParserException("There was an error idenfitying the Attribute Type. Is the Type '" + args[3] + "' supported?\nThe list of supported protocols is:\n" + Arrays.toString(AttrType.values()));
		} catch (IndexOutOfBoundsException e){
		...
		try{
			...
			throw new CommandParserException("There was an error idenfitying the Data Type. Is the Type '" + args[4] + "' supported?\nThe list of supported protocols is:\n" + Arrays.toString(DataType.values()));
		} catch (IndexOutOfBoundsException e){
